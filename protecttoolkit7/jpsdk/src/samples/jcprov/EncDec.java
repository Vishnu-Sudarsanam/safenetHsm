 ...
 * <li><i>keytype</i>  one of (des, des2, des3, aes, rsa)
...
public class EncDec
{
    ...
    public static void usage()
    {
        println("java ...EncDec -keyType <keytype> -keyName <keyname> " +
                "[-slot <slotId>] [-password <password>]");
        println("");
        println("<keytype>  one of (des, des2, des3, aes, rsa)");
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            else if (keyType.equalsIgnoreCase("aes"))
            {
                ...
                hKey = findKey(session, CKO.SECRET_KEY, CKK.AES, keyName,
                        bPrivate);

                if (!hKey.isValidHandle())
                {
                    println("aes key (" + keyName + ") not found");
                    return;
                }

                // GCM encryption
                // Use an IV of all zeros to force the HSM to generate a new random value
                byte[] iv = new byte[]  { 0, 0, 0, 0, 0, 0, 0, 0 };
                byte[] aad = new byte[] { 0x10, 0x32, 0x43, 0x65, 0x78, (byte)0x9a, 0x01, 0x23 };

                CK_GCM_PARAMS gcmparams = new CK_GCM_PARAMS(iv, aad, 64);
                CK_MECHANISM mech = new CK_MECHANISM(CKM.AES_GCM, gcmparams);

                println("AES GCM operation");
                symetricEncDec(session, hKey, mech );

                // GCM encryption
                byte[] cb = new byte[]  { 1, 2, 3, 4, 5, 6, 7, 8, 0, 0, 0, 0, 0, 0, 0, 0 };

                CK_AES_CTR_PARAMS ctrparams = new CK_AES_CTR_PARAMS(cb, 64);
                mech = new CK_MECHANISM(CKM.AES_CTR, ctrparams);

                println("AES CTR operation");
                symetricEncDec(session, hKey, mech );
            }
                               ...
                               CK_MECHANISM mechanism)
    {
        ...
        byte[] plainText = startString.getBytes(StandardCharsets.US_ASCII);
        ...
        String endString = new String(plainText, 0, (int)lRefDec.value, StandardCharsets.US_ASCII);
