 ...
 * <p>
 * This progams initialised the Cryptoki library according to the specified locking
 ...
 * <p>
 * Usage : java ...Threading -numThreads &lt;numthreads&gt; -keyName &lt;keyname&gt; -locking &lt;lockingmodel&gt; -duration &gt;duration&lt; -userPin &gt;userpin&lt; [-v]
 ...
 * <li><i>userpin</i>       user pin for slot 0
 * <li>-v                    verbose mode
 */
public class Threading implements Runnable
{
     ...
     */
    static class CustomCreateMutex implements CK_C_INITIALIZE_ARGS.CK_CREATEMUTEX
    {
     ...
     */
    static class CustomDestroyMutex implements CK_C_INITIALIZE_ARGS.CK_DESTROYMUTEX
    {
     ...
     */
    static class CustomLockMutex implements CK_C_INITIALIZE_ARGS.CK_LOCKMUTEX
    {
     ...
     */
    static class CustomUnlockMutex implements CK_C_INITIALIZE_ARGS.CK_UNLOCKMUTEX
    {
    ...
    {
        println("java ...Threading -numThreads <numthreads> -keyName <keyname> -locking <lockingmodel> -duration <duration> -userPin <userpin> [-v]");
        println("");
        ...
        println("<userPin>      the user pin for slot 0");
        ...
        try
        {
            ...
            else if (bFunctions)
            {
                ...
                CryptokiEx.C_Initialize(
                        new CK_C_INITIALIZE_ARGS(new CustomCreateMutex(),
                                                 new CustomDestroyMutex(),
                                                 new CustomLockMutex(),
                                                 new CustomUnlockMutex())
                );
            }
            else
            {
            ...
            CryptokiEx.C_OpenSession(_slotId, CKF.RW_SESSION, null, null, session);

            /* Login to the session if a user pin has been provided */
            if (_bLogin)
            {
                CryptokiEx.C_Login(session, CKU.USER, userPin.getBytes(StandardCharsets.US_ASCII), userPin.length());
            }

            /* Locate the key to use */
            _hKey = new CK_OBJECT_HANDLE();
            CTUtilEx.CTU_FindObject(session, CKO.SECRET_KEY, keyName.getBytes(StandardCharsets.US_ASCII), _hKey);
    ...
    public void run()
    {
        ...
        byte[] data = "This is 16 Bytes".getBytes(StandardCharsets.US_ASCII);
        byte[] cipher = null;
